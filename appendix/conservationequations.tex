...
\end{equation}
where $\hat{E}$ (J\,kg$^{-1}$), the intensive property is related total energy $E$ (J), the extensive property. The total energy per unit mass of the mixture $\hat{E}$ is given as: 
\begin{equation}
...
e = h - RT = h - \frac{p}{\rho}
\end{equation}
where $R$ (J\,kg$^{-1}$\,K$^{-1}$) is the gas constant. The total enthalpy $h$ (J\,kg$^{-1}$) of the gas mixture is defined in \cref{app:thermodynamics}, i.e., \cref{enthalpymoistair} and given as:
 \begin{equation}
...
\begin{equation}
\mvec{q} = \mvec{q}_c + \mvec{q}_d = \lambda \nabla T + \sum_i h_i\, \mvec{g}_{\textit{d,i}}
\label{eq:totalheatflux}
\end{equation}
where $\mvec{q}_c$ (W\,m$^{-2}$) is the Fourier law of heat conduction, with:
\begin{equation}
\lambda = - \frac{c_p \mu}{\mathrm{Pr}}
\end{equation}
and $\mvec{g}_{\textit{d,i}}$ (kg\,m$^{-2}$\,s$^{-1}$) diffusive flux of species $i$ contributing to the heat transport. Simplifying \cref{eq:energy1}, we obtain:
\begin{equation}
...
\begin{equation}
s_h = a\,\left(q_{\textit{sen,leaf}} + \sum_i h_i g_{\textit{i,leaf}} \right)
\label{eq:shterm1}
\end{equation}
where $a$ (m$^{2}$\,m$^{-3}$) is leaf area density, $q_{\textit{sen,leaf}}$ (W,m$^{-2}$) heat flux due to heat conduction from the vegetation, $ g_{\textit{i,leaf}} $ is the mass flux from vegetation. And so, 
\begin{equation}
s_h = a\, \left(q_{\textit{\textit{cond},leaf}} + h_v g_{\textit{v,leaf}} + h_c g_{\textit{c,leaf}} \right)
\label{eq:shterm2}
\end{equation}
where $g_{\textit{v,leaf}}$ (kg\,m$^{-2}$\,s${-1}$) and $g_{\textit{c,leaf}}$ (kg\,m$^{-2}$\,s${-1}$) are fluxes of water vapor and CO$_2$. 

\begin{assumption}
We assume there is no flux of oxygen O$_2$. Thus, substituting the enthalpies of water vapor and CO$_2$, we get:
\end{assumption}

The sensible heat flux $q_{\textit{sen,leaf}}$ (W\,m$^{-2}$), i.e., the heat flux due to conduction from the leaf is given as:
\begin{equation}
q_{\textit{cond},leaf} = h_{\textit{c,h}} \left(T_l - T\right)
\end{equation}
...
\end{equation}
where $g_{\textit{v,leaf}}$ (kg\,m$^{-2}$\,s$^{-1}$) is the leaf CO$_2$ assimilation rate. Therefore, we can divide the source of energy \cref{eq:shterm2} into sensible and latent heat flux components as:
\begin{equation}
...
Thus, the source of energy $s_h$ (W\,m$^{-3}$) becomes:
\begin{equation}
...
\begin{split}
&c_{\textit{pa}}   \left( \frac{\partial }{\partial t} \left( \rho_a T \right) + \nabla \cdot \left(\rho_a T \mvec{u}_a \right)  \right) + c_{\textit{pv}} \left( \frac{\partial }{\partial t} \left( \rho_v T \right) + \nabla \cdot \left(\rho_v T \mvec{u}_v \right)  \right) \\ 
&+ L_v \left(a\,g_{\textit{v,leaf}}  \right) = - \nabla  \cdot \left(\lambda \nabla T \right) + a \left( h_{\textit{c,h}} \left(T_l - T\right) + L_v g_{\textit{v,leaf}} \right)
\end{split}
\label{eq:energy10}
\end{equation}
and we can cancel the latent component on both sides, giving:
\begin{equation}
\begin{split}
&c_{\textit{pa}}   \left( \frac{\partial }{\partial t} \left( \rho_a T \right) + \nabla \cdot \left(\rho_a T \mvec{u}_a \right)  \right) + c_{\textit{pv}} \left( \frac{\partial }{\partial t} \left( \rho_v T \right) + \nabla \cdot \left(\rho_v T \mvec{u}_v \right)  \right) \\ 
...
So, we see that following the assumption, the source of energy is simply due to conductive sensible heat flux from the temperature difference between air and leaf, i.e., $s_h$ (W\,m$^{-3}$):
\begin{equation}
{s_h} = a h_{\textit{c,h}} \left(T_l - T\right) = a\,q_{\textit{sen,leaf}}
\label{eq:shterm5}
\end{equation}

%\begin{equation}
%s_h = a\,\left(2\,h + g_{\textit{v,leaf}} \left(c_{pv}(T-T_\textit{ref})\right) + h_c g_{\textit{c,leaf}}\right)
%\label{eq:shterm3}
%\end{equation}
...
Combining, the density of air and water vapor, $\rho = \rho_a + \rho_v$ and $c_p = c_{\textit{pa}} + c_{\textit{pv}}$, \cref{eq:energy11} can be written in the form:
\begin{equation}
c_p  \left( \frac{\partial }{\partial t} \left( \rho T \right) + \nabla \cdot \left(\rho T \mvec{u} \right)  \right) = - \nabla  \cdot \left(\lambda \nabla T \right) + a\,q_{\textit{sen,leaf}}
\label{eq:energy12}
\end{equation}
