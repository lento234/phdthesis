...
\begin{equation}
\frac{\partial \rho}{\partial t} + \nabla \cdot \left( \rho \mvec{u} \right) = s_w
\end{equation}
where $\rho$ is defined by \cref{eq:rho2} and as $\mathcal{O}(s_w)\approx \num{e-4}$ (\cref{app:conservation}), we assume source of water vapor is negligible in conservation of mass.
...
\end{equation}
and the source of energy $s_h$:
\begin{equation}
\mvec{s}_h = \rho\,c_p\, a\, \left(q_{\textit{sen,leaf}} + q_{\textit{lat,leaf}} \right)
\label{eq:sh}
\end{equation}
assuming that the sensible heat flux is only due to leaf and air. The sensible heat flux associated to the source of moisture is assumed to be negligible. 

The enthalpy of dry air and water vapor are:
\begin{align}
h_a &= c_{p,a} \left(T - T_{\mathit{ref}}\right) \\
h_v &= c_{p,v} \left(T - T_{\mathit{ref}}\right) + L_v \left(x_v - x_{v,ref}\right)
\end{align}
where water vapor contains the latent heat of vaporization. Thus, the total enthalpy of moist air is:
\begin{equation}
h = h_a \, x_a + h_v \, x_v
\end{equation}
and so:
...
The radiative heat flux absorbed by the leaf ${q_{\mathit{rad,leaf}}}$ (W\,m$^{-3}$) in \cref{eq:energybalance2} is defined as:
\begin{equation}
...
where $q_{\textit{s,0}}$ (W\,m$^{-2}$) is the short-wave radiative flux above vegetation, and $\beta$ is the extinction coefficient for short-wave radiation. Figure \cref{fig:radiation_shortwave} depicts the decay of a single ray along the path $r$ with solar altitude $\phi$. Therefore, to determine the extinction of short-wave radiation within the foliage, the leaf area density $a$ is integrate along the path $r$. The path $r$ depends on the solar altitude $\phi$ at a given time of the day.
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.5\textwidth]{\figdir/radiation_longwave-crop.pdf}
...
The long-wave radiative heat flux $\mvec{q}_r$ component absorbed by vegetation (in \cref{eq:qradleaf2}), i.e.:
\begin{equation}
\nabla \cdot \mvec{q}_r 
\end{equation}
