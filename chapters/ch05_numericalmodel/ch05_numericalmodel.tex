...
\end{equation}
where $\overline{\rho \mvec{u}' \mvec{u}'}$ is the Reynolds stress tensor. The Reynolds stresses are modeled using Bousinessq eddy-viscosity assumption:
\begin{equation}
...
In this study, we employ realizable $k-\varepsilon$ equations for determining the eddy-viscosity \citep{Shih1995}. The system of equations is given as:
\begin{align}
...
q_{\textit{rad,r,k}} &= \epsilon_k \sigma T_k^4 - \sum_{i=1}^{N}  \frac{A_i}{A_k} \epsilon_i \sigma T_i^4 G_{i,k}
\label{eq:qradsurf}
\end{align}
...
\begin{enumerate}
	...
	\item Repeat steps (2) to (4) until scaled residuals have reached the convergence limit of $\epsilon < \num{e-6}$.
\end{enumerate}
