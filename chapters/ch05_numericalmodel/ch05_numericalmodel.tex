...
\end{equation}
where $\overline{\rho \mvec{u}' \mvec{u}'}$ is the \highlight{Reynolds stress tensor}. The Reynolds stresses are modeled using Bousinessq eddy-viscosity assumption:
\begin{equation}
...
\begin{enumerate}
	...
	\item Repeat steps (2) to (4) until \highlight{scaled residuals} have reached the convergence limit of $\epsilon < \num{e-6}$.
\end{enumerate}
