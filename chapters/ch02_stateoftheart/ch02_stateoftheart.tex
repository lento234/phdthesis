...
%A study on the energy budget in an urban environment can provide a initial understanding of the coupling between varies processes in such an environment.
	...
	\begin{figure}[t]
		...
		\caption{Time and length scale of phenomena in urban climate \citep{Oke2017a}. \hl{Make new diagram}}
...
A study on the energy budget in an urban canopy layer can provide an initial understanding of the coupling between various processes in the urban microclimate. The energy budget can be determined by applying the conservation of energy for the UCL. The heat sinks and sources inside the domain are balanced by the fluxes at the boundaries. Figure \ref{fig:energybalance} shows a schematic representation of the energy balance applied to the UCL. 
	...
	\begin{figure}[h]
		...
		\caption{The energy balance of an urban canopy layer (UCL) \citep{Oke2017a}. \hl{Fix symbols inline with Jan/Lab}, \hl{Make new diagram}}
...
Urban greening solutions is seen as an effective tool to combat the growing UHI and improve the climate in urban areas \citep{Gillner2015, Bowler2010, Loughner2012}.  The energy contribution of the latent heat flux from vegetation in urban microclimate cannot be neglected as seen in section \ref{subsec:energybalance}. Furthermore, urban vegetation has a direct influence on the aerodynamic, radiative, thermal and moisture properties of the urban microclimate \citep{Oke1989}. Thus, the interaction of urban vegetation with the environment can be seen as the coupling between the radiative, heat and moisture balance, as depicted in Fig. {fig:energymassbudget}. Therefore, to accurately assess the impact of urban vegetation on the urban microclimate, these coupled processes must be taken into account.
	...
	\begin{figure}[h]
		...
		\caption{Coupling between radiative, energy and water budget \citep{Sawinski2011}. \hl{Remove carbon budget, not in scope}}.
...
\begin{figure}[h]
	...
	\caption{Decomposition of the energy spectrum into two distinct parts: short-wave radiation $Q_{r,sw}$ and long-wave radiation $Q_{r,lw}$ \citep{Oke2017a}.  \hl{Make a new diagram}}
...
An important variable used to define the state of water in various domain (ex. soil, plant, air) is the water potential $\psi$. Water travels from high water potential to low water potential, and so, the transport of water can be determined merely from the water potential gradient. It is formulated as follows:
\begin{equation}
\mvec{g} = k\ \nabla \psi
\end{equation}
...
The goal of the present work is to address the gap found in the state of the art. There is a need of a rigorous understanding on the impact of transpiration from vegetation in an urban microclimate using a detailed model with a direct study on the thermal comfort. Such study can provide an understanding on the cooling potential of vegetation and the potential mitigation factor. For this, a three-dimensional computational fluid dynamics (CFD) model of heterogeneous, porous vegetation is needed to be developed, which calculates the turbulent momentum, heat, and mass exchanges of vegetation with the urban environment. Furthermore, an important factor driving the transpiration rate is the water availability at the root zone. Therefore, the numerical model should be coupled with a soil-plant continuum model to capture the full water transport cycle. This enables to investigate the water cycle between atmosphere to the soil and then back to the environment via the vegetation. More importantly, the influence of water stress on the urban microclimate and its implication on the thermal comfort needs to be addressed. The developed modeling framework can be used to investigate the response of vegetation in an urban environment for various climate conditions, built and vegetation parameters.
